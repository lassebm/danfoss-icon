...
class DanfossIconProblem final : public binary_sensor::BinarySensor, public Component, public DanfossIconListener {
 public:
  ...
  void on_attr(uint8_t idx, uint16_t attr_id, const uint8_t *data, size_t len) override {
    if (idx != this->idx_ || attr_id != DI_ATTR_ERROR_CODE || len < 2)
      return;
    ...
    uint16_t mask = (this->idx_ >= 0x31) ? 0x0D00u : 0xFF02u;

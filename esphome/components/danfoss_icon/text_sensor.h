...
enum DiTextDecode {
  ...
  DI_TEXT_HEX = 2,         // raw bytes as uppercase hex
  ...
  DI_TEXT_REGSENSOR = 10,  // 0x030A floor-sensor mode -> "Comfort" / "Floor" / "Dual"
};
...
class DanfossIconTextSensor : public text_sensor::TextSensor, public Component, public DanfossIconListener {
 ...
 protected:
  ...
  uint16_t og_slow_{0}, og_med_{0}, og_fast_{0};  // cached output-group words (DI_TEXT_OUTPUTS)
};
